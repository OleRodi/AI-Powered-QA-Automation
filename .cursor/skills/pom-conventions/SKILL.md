...
5. **Page URLs** — full pages extend `BasePage`, which reads `DIDAXIS_URL`
   from the environment. Use `` `${this.baseURL}/programs` `` in `goto()`.
...
| Test | Guardrail |
|------|-----------|
| DS-2 TC-012 | Duplicate program names on rename are **allowed** (hardcoded demo bug). Mark with `test.fail(true, 'Known demo bug — duplicate program names are allowed on rename.')`. |
